...
- [x] Inventory every Overview metric from its label through its SQL/RPC source. Record the numerator, denominator, time window, null/empty behavior, row-level access filter, and plan limit for each.
- [x] Remove misleading aliases and unsupported display entries, including â€œAutomation Scoreâ€/`automation_offload` unless the server can attribute the measured events to automation.
- [x] Replace `value || 0` display fallbacks for observed analytics with explicit unavailable/empty rendering; retain `0` only when the source returned a measured zero.
...
- [x] Add or update focused pure/data-contract checks for zero, null, empty cohorts, and permission-filtered rows. Do not change server authorization behavior.
...
**Metric truth inventory (2026-09-23):** All audit metrics use the filtered `base_tasks` cohort: caller company, non-deleted tasks, tasks created inside the selected date window, plus optional pipeline/project/priority/team/worker filters. The restored public wrapper requires `analytics.view`, fixes the caller to `auth.uid()`, and rejects foreign or deleted pipeline IDs. Billing limits control client presentation after billing resolves; they do not replace database authorization.

| UI metric | RPC field | Definition and empty behavior | Plan/presentation |
| --- | --- | --- | --- |
| Completed successfully | `current.throughput` | Count of cohort tasks whose current stage is a successful terminal stage. An empty cohort is a measured count of `0`. | Overview presentation is gated by resolved analytics limits; the RPC still requires `analytics.view`. |
| Success rate | `current.success_rate` | Successful terminal cohort tasks divided by all cohort tasks. Empty cohort is `null`; a non-empty cohort with no successes is `0`. `current.sample_size` is the denominator. | Render the ring only when `sample_size > 0`; preserve real zero. |
| Average lead time | `current.avg_lead_time_minutes` | Average `completed_at - created_at` for cohort tasks currently in a terminal stage with a completion timestamp. No eligible completed task is `null`; a measured zero duration remains `0`. | Detailed Performance metric; unavailable must not become zero. |
| Revision rate | `current.revision_rate` | Cohort tasks with at least one `needs_revision` or `rejected` submission divided by all cohort tasks. Empty cohort is `null`; measured zero remains `0`. | Detailed Performance metric; definition remains task based. |
| First-pass integrity | `radar_advanced.first_pass_yield` | Currently cohort tasks without a revision/rejection submission divided by all cohort tasks, including open work. Empty cohort is `null`. | Do not promote until product accepts an eligible completed-work denominator and matching copy. |
| Flow ratio | `radar_advanced.flow_ratio` | Currently duplicates success rate rather than measuring flow efficiency. | Removed from renderable Overview entries; server-field disposition is a product decision. |
| Automation offload | `radar_advanced.automation_offload_rate` | Currently a constant `0`; no automation attribution source exists. | Never render as a measured KPI; server-field disposition is a product decision. |

**Open product decisions:** Decide whether to remove or null `automation_offload_rate`, whether the duplicate `flow_ratio` remains an internal compatibility field, and whether First-pass integrity should use completed terminal tasks as its eligible denominator. No implementation should invent these definitions.
...
**Audit empty-sample semantics (2026-09-23):** The migration changes only `current.success_rate`, `current.avg_lead_time_minutes`, `current.revision_rate`, `comparison.success_rate`, `comparison.avg_lead_time_minutes`, `comparison.revision_rate`, and `radar_advanced.first_pass_yield`: empty samples now return null, while measured zero with an eligible sample remains zero. It also returns `current.sample_size` as the count of filtered `base_tasks`; the client types this field as optional so an older server payload hides the outcome ring instead of guessing whether a zero is measured. The migration is deployed as remote version `20260923072110`. Its first rollback-only check exposed that the live public function was still the pre-wrapper body with an anon grant; the fixture transaction rolled back. ACL hardening (`20260923072337`) and wrapper restoration (`20260923072603`) repaired the public authorization boundary, after which the audit empty/null, measured-zero, permission, tenant, ACL, and fixed-search-path check passed with zero fixture-company leftovers.
...
- [x] Preserve permission-denial and server-error states as unavailable/access states; never turn either into plausible zero values.
...
**Phase 3 progress (2026-09-23):** Desktop and adaptive Overview now share `AtAGlance`: a Completed successfully total plus an optional Success rate ring with plain labels. The ring includes the filtered task sample count and preserves a valid measured 0%. Configurable KPI tiles and their saved widget preferences were removed. A separate Target Watch uses the canonical hook, is omitted without `target.view`, and shows up to three active volume targets with loading, unavailable, and empty states kept distinct. Overview fetches now use keyed generations, discard stale responses after scope changes, distinguish initial loading/updating/empty/error, and expose Retry without leaving prior-scope data looking current. The audit empty-sample migration and authorization wrapper are deployed and SQL-verified. Runtime, visual-width, and browser verification remain open.
...
- [x] Keep the `analytics.view` permission check separate from plan entitlement: the client uses permission for navigation/presentation, while each protected server reader continues to enforce the permission and per-row access itself.
  ...
  - [x] Apply the migration and run SQL regression checks proving both RPCs allow a same-company live pipeline, reject anon and callers without `analytics.view`, raise `42501` for foreign-company/deleted/missing pipelines, and deny direct execution of both private delegates.
...
**Phase 4 remaining audit (2026-09-23):** Archives remains a separate permission-gated destination and its server readers retain principal/company scope. The desktop Intelligence sidebar now omits Reports until billing is resolved and entitled. Desktop Overview and desktop/adaptive Performance keep permission loading/denial distinct from billing loading/error and resolved plan access. Direct Performance routes require `analytics.view`; direct Targets routes preserve the separate `target.view` contract, and neither mounts its data-owning content before permission resolution. Adaptive legacy `?section=analytics` and `?section=targets` replace to their routes; `radar` and permission-allowed `archives` remain local, while unknown or unauthorized section values fall back to Overview. The bucketed-range tenant wrapper migration is deployed as remote version `20260923072220`; its corrected rollback-only check passed same-company access, all denial cases, private-delegate isolation, wrapper ACL/config, and rollback cleanup. The remaining Phase 4 item is the broader information-architecture checkbox and any target-dependent navigation choice, not server scope hardening.
...
**Database verification follow-up (2026-09-23):** Audit empty-sample, bucketed-range tenant-scope, audit ACL hardening, and audit wrapper restoration are recorded remotely as versions `20260923072110`, `20260923072220`, `20260923072337`, and `20260923072603`. The first audit check exposed the missing live wrapper/anon ACL and rolled back; the first bucketed check exposed invalid test-only dynamic SQL and rolled back. After the source corrections, both rollback-only checks passed. Live inspection confirms all three public wrappers are `SECURITY DEFINER`, `search_path=public`, and executable only by `authenticated`; all three private delegates deny `authenticated`, `anon`, and `service_role`. The audit check also proves a successful permitted global read excludes a mixed foreign-company task. Fixture-company cleanup returned zero.

**Final focused source verification (2026-09-23):** The analytics metric, target adapter, canonical hook, and throughput presentation suites pass 20/20 across four files. Targeted Babel parsing passes for Overview, Analytics Pipeline, Performance, Targets, and `RadarWidgets` on desktop/adaptive variants, and the scoped diff check is clean. `npm run verify:agent` was attempted but cannot serve as a green repository gate in this dirty multi-worktree state: unrelated FileHub/defaults-catalog checks fail, and `Block.check.ts` still expects the adaptive Overview to import `Block` after the density refactor removed those detail blocks. The gate was stopped after the failures were established rather than waiting through every unrelated worktree. Browser/mobile walkthrough boxes remain unchecked; no runtime or visual claim is made from source checks.
