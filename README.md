...
Contributions are welcome and are accepted on a voluntary, unpaid basis. See [CONTRIBUTING.md](./CONTRIBUTING.md) for branching conventions (branch off `experimental`, not `master`) and contribution terms before opening a PR.

## License

PolyForm Noncommercial 1.0.0 — free to use, modify, and redistribute for noncommercial purposes. Commercial use requires written permission from the copyright holders. See [LICENSE](./LICENSE) for full terms.

© 2026 Adam Samir (abysrift) and Mostafa Eid (lleoparden).
