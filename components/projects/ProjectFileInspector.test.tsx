...
const storage = vi.hoisted(() => ({ openStorageFile: vi.fn() }));
...
describe('ProjectFileInspector', () => {
  ...
  it('renders signed image/document previews and permitted version actions', async () => {
    ...
    viewerState.handlePress.mockClear();
    storage.openStorageFile.mockClear();
    ...
    expect(viewerState.handlePress).toHaveBeenCalledWith(expect.objectContaining({ id: 'doc', bucket: 'project-files', storagePath: 'doc.pdf' }));
    await press(renderer, 'Download');
    expect(storage.openStorageFile).toHaveBeenCalledWith('project-files', 'doc.pdf', 'doc.pdf', 'application/pdf');
    await press(renderer, 'versions');
    expect(texts(renderer)).toEqual(expect.arrayContaining(['Download', 'Restore']));
    await press(renderer, 'Download', 1);
