...
import ExplorerUploadAction from '@/components/filehub/explorer/ExplorerUploadAction';
import ProjectFileInspector from '@/components/projects/ProjectFileInspector';
...
export default function ProjectFilesTab({ folderParam, fileParam }: ProjectFilesTabProps = {}) {
  ...
  const {
    ...
    deleteProjectFile, restoreProjectFolder, restoreProjectFile, projectFileVersions,
    restoreProjectFileVersion, fileActivity, logActivity,
  } = useFileHub();
  ...
  useEffect(() => { if (!loading && envelope && !envelope.workspace?.root && envelope.workspace?.capabilities?.create && !ensured.current) { ensured.current = true; void ensureProjectWorkspace(projectId).then(refresh).catch(() => { ensured.current = false; }); } }, [loading, envelope, ensureProjectWorkspace, projectId, refresh]);
  const workspace = envelope?.workspace; const projectCapabilities = workspace?.capabilities || { view: false, create: false, rename: false, move: false, delete: false, restore: false, upload: false, replace: false, version: false }; const root = workspace?.root as ProjectFileHubFolder | null; const folders = (workspace?.folders || []) as ProjectFileHubFolder[]; const files = (workspace?.files || []) as ProjectFileHubFile[]; const currentFolder = folders.find(f => f.id === selectedFolderId) || root; const canCreate = projectCapabilities.create === true; const canRename = projectCapabilities.rename === true; const canMove = projectCapabilities.move === true; const canDelete = projectCapabilities.delete === true; const canRestore = projectCapabilities.restore === true; const canMutate = canCreate || canRename || canMove || canDelete;
  ...
  const runDialog = async () => { const value = dialogValue.trim(); if (!value || !currentFolder) return; try { if (dialog === 'create' && canCreate) await createProjectFolder(projectId, value, currentFolder.id); else if (dialog === 'rename' && canRename && selectedFolderId) await renameProjectFolder(projectId, selectedFolderId, value); else return; setDialog(null); setDialogValue(''); successToast(dialog === 'create' ? 'Folder created.' : 'Folder renamed.'); await refresh(); } catch (error: any) { errorToast(error?.message || 'Folder action failed.'); } };
  ...
  const deleteItem = (item: WorkspaceItem) => { if (!canDelete) return; return showConfirm(
  ...
  return <View className="flex-1 gap-4 p-4 md:p-8"><View><Text className="text-typography-main text-lg font-black">Project files</Text><Text className="mt-1 text-typography-muted text-xs">Working files stay editable here. Client inputs and sealed outputs remain separate and protected.</Text></View>{binBlock}<View className="flex-1 gap-4 md:flex-row"><View className="gap-4 md:w-[28%]"><ReferenceStrip title="Client standing files" hint="Shared reference material that persists across projects." files={envelope.standing_files} /><ReferenceStrip title="Sealed deliverable" hint="Project output, versioned and read-only." files={envelope.deliverable_files} sealed /><SealedVersionHistory versions={envelope.deliverable_versions || []} /></View><View className="min-h-[420px] flex-1"><ExplorerInspectorShell navigation={tree} collection={filesBlock} inspector={selectedFile ? <ProjectFileInspector file={selectedFile} projectId={projectId} capabilities={projectCapabilities} onRefresh={refresh} onClose={isMobile ? () => { setSelectedFile(null); setMobilePage('files'); } : undefined} projectFileVersions={projectFileVersions} restoreProjectFileVersion={restoreProjectFileVersion} fileActivity={fileActivity} logActivity={logActivity} /> : undefined} mobilePane={selectedFile ? 'inspector' : 'collection'} onRequestCollection={() => { setSelectedFile(null); setMobilePage('files'); }} /></View></View><Popup visible={!!moveItem} onClose={() => setMoveItem(null)} presentation="auto" maxWidth={420} title={`Move ${moveItem?.name || 'item'}`}><View className="max-h-[420px] gap-1.5 p-5">{moveCandidates.map(folder => <TouchableOpacity key={folder.id} onPress={() => void runMove(folder)} className="min-h-[44px] flex-row items-center gap-2 rounded-xl border border-surface-border px-3"><FontAwesome name="folder-o" size={12} color={colors.textMuted} /><Text className="flex-1 text-typography-main text-xs">{folder.name}</Text></TouchableOpacity>)}{moveCandidates.length === 0 && <Text className="text-typography-muted text-xs">No valid workspace destination.</Text>}</View></Popup><Popup visible={!!dialog} onClose={() => setDialog(null)} presentation="auto" maxWidth={420} title={dialog === 'create' ? 'Create folder' : 'Rename folder'}><View className="gap-4 p-5"><TextInput autoFocus value={dialogValue} onChangeText={setDialogValue} placeholder="Folder name" placeholderTextColor={colors.textMuted} className="rounded-lg border border-surface-border bg-surface-background px-3 py-3 text-typography-main" /><View className="flex-row justify-end gap-2"><Action icon="times" label="Cancel" onPress={() => setDialog(null)} /><Action icon="check" label="Save" onPress={runDialog} disabled={!dialogValue.trim()} /></View></View></Popup></View>;
}
