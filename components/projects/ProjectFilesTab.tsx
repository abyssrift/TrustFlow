...
import MultiViewList, { type MultiViewColumn } from '@/components/common/MultiViewList';
...
import type { ProjectFileHubBin, ProjectFileHubFile, ProjectFileHubFolder, ProjectFileHubReference } from '@/lib/projectFileHubNormalization';
...
export default function ProjectFilesTab() {
  ...
  const {
    ...
    deleteProjectFile, restoreProjectFolder, restoreProjectFile, projectUploadTarget,
  } = useFileHub();
  ...
  const upload = async () => { if (!currentFolder || !workspace?.capabilities?.upload || !profile?.company_id) return; const result = await DocumentPicker.getDocumentAsync({ type: '*/*', multiple: true, copyToCacheDirectory: true }); if (result.canceled) return; const managerFiles = await Promise.all(result.assets.map(async asset => { const blob = await (await fetch(asset.uri)).blob(); return new File([blob], asset.name, { type: asset.mimeType || blob.type || 'application/octet-stream' }); })); const jobId = startUpload({ files: managerFiles, companyId: profile.company_id, visibility: 'project', folderId: currentFolder.id, recipientIds: [], groupId: null, tags: [], caption: null, maxFileSizeBytes: null, scopedFolders: [], target: projectUploadTarget(projectId, currentFolder.id), label: 'Project files' }); await waitForUpload(jobId); await refresh(); };
