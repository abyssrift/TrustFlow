...
export default function ProjectFileInspector({ file, projectId, canRestore, onRefresh, onClose, projectFileVersions, restoreProjectFileVersion, fileActivity, logActivity }: ProjectFileInspectorProps) {
  ...
  const restore = (version: ProjectVersion) => showConfirm('Restore version?', `Restore ${file.name} to version ${version.version_no}?`, async () => { await restoreProjectFileVersion(projectId, version.id); setVersions(null); onRefresh(); });
  return <>
    <ExplorerDetailPane
      ...
      capabilities={{ canView: true, canCreate: false, canRename: false, canMove: false, canDelete: false, canRestore, canUpload: false, canReplace: false, canVersion: true }}
      onClose={onClose}
