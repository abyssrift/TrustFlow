...
export default function ExplorerDetailPane<T extends import('./ExplorerTypes').ExplorerItem>({ item, capabilities, onClose, onOpen, onDownload, onShare, children, renderHeader, renderActions }: ExplorerDetailPaneProps<T>) {
  ...
  return <View className="flex-1 overflow-hidden rounded-2xl border border-surface-border bg-surface-card">
    <View className="flex-row items-start gap-3 border-b border-surface-border p-4">{renderHeader ? renderHeader(item) : <View className="flex-1"><Text numberOfLines={2} className="text-typography-main text-base font-black">{item.name}</Text>{item.mimeType && <Text className="mt-1 text-typography-muted text-xs">{item.mimeType}</Text>}</View>}{onClose && <TouchableOpacity accessibilityLabel="Close details" onPress={onClose} className="h-11 w-11 items-center justify-center rounded-xl border border-surface-border"><FontAwesome name="times" size={12} className="text-typography-muted" /></TouchableOpacity>}</View>
    {canView && <View className="flex-row flex-wrap gap-2 border-b border-surface-border p-4">{renderActions ? renderActions(item) : <>{action('Open', 'external-link', onOpen)}{action('Download', 'download', onDownload)}{action('Share', 'share', onShare)}</>}</View>}
    <ScrollView className="flex-1" contentContainerClassName="gap-4 p-4">{children || <><Detail label="Path" value={item.canonicalPath || item.path} /><Detail label="Project" value={item.projectName} /><Detail label="Size" value={item.sizeBytes == null ? null : `${item.sizeBytes} bytes`} /><Detail label="Origin" value={item.origin} /><Text className="text-typography-dim text-[10px]">{capabilities?.canView ? 'Available in this explorer.' : 'Read access is not granted.'}</Text></>}</ScrollView>
  </View>;
}
