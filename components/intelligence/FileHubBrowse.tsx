...
import { canonicalIdentityKey, getBrowseOriginLabel, getBrowsePageCursor, groupByCanonicalIdentity } from './filehubShared';
...
export default function FileHubBrowse({ compact }: { compact?: boolean }) {
  ...
  const fetchPage = useCallback(async (before: string | null, withFacets: boolean) => {
    ...
    if (error) { console.error('[FileHubBrowse] error', error); return { items: [] as BrowseItem[], has_more: false, facets: null, cursor: before }; }
    const result = data as { items: BrowseItem[]; has_more: boolean; facets: Facets | null };
    const rawItems = result.items ?? [];
    return { ...result, items: groupBrowseItems(rawItems), cursor: getBrowsePageCursor(rawItems, before) };
  }, [searchDebounced, sources, projectId, origin, category, type]);
  const [pageCursor, setPageCursor] = useState<string | null>(null);
  useEffect(() => { let cancelled = false; setLoading(true); setPageCursor(null); setDetail(null); setSelectedIds(new Set()); setAnchorIdx(null); fetchPage(null, true).then(result => { if (!cancelled) { setItems(result.items); setPageCursor(result.cursor); setHasMore(result.has_more); setFacets(result.facets); setLoading(false); } }); return () => { cancelled = true; }; }, [fetchPage]);
  const loadMore = async () => { if (loadingMore || !items.length || !pageCursor) return; setLoadingMore(true); try { const result = await fetchPage(pageCursor, false); setItems(previous => groupBrowseItems([...previous, ...result.items])); setPageCursor(result.cursor); setHasMore(result.has_more); } finally { setLoadingMore(false); } };
