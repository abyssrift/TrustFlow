...
import { canonicalIdentityKey, getBrowseOriginLabel } from './filehubShared';

export type BrowseItem = {
  source: 'filehub' | 'submission' | 'task_brief'; file_id: string;
  canonical_file_id: string | null; canonical_version_id: string | null;
  workspace_folder_id: string | null; workspace_path: string | null;
  origin: 'workspace' | 'deliverable' | 'shared' | 'brief' | 'submission' | null;
  bucket: string; storage_path: string; file_name: string; mime_type: string | null;
  size_bytes: number; created_at: string; task_id: string | null; task_title: string | null;
  project_id: string | null; project_name: string | null; task_category: string | null;
  submission_id: string | null; uploaded_by?: string | null;
  alias_count?: number; alias_context?: string;
};
...
const PAGE = 60;
const ORIGIN_OPTIONS = ['workspace', 'deliverable', 'shared', 'brief', 'submission'];
const SOURCE_TABS = [{ label: 'All', value: null }, { label: 'FileHub', value: ['filehub'] }, { label: 'Submissions', value: ['submission'] }, { label: 'Briefs', value: ['task_brief'] }] as const;
const idOf = (item: BrowseItem) => canonicalIdentityKey(item);
const isImage = (mime: string | null) => !!mime && mime.toLowerCase().includes('image');
...
export function groupBrowseItems(rows: BrowseItem[]): BrowseItem[] {
  ...
  for (const row of rows) groups.set(canonicalIdentityKey(row), [...(groups.get(canonicalIdentityKey(row)) ?? []), row]);
  return [...groups.values()].map(group => {
    const representative = group[0];
    const contexts = [...new Set(group.map(row => [row.origin && getBrowseOriginLabel(row.origin), row.project_name || row.task_title].filter(Boolean).join(' · ')))].filter(Boolean);
    return { ...representative, alias_count: Math.max(0, group.length - 1), alias_context: contexts.length > 1 ? contexts.join(' · ') : undefined };
  });
}
...
export default function FileHubBrowse({ compact }: { compact?: boolean }) {
  ...
  const [items, setItems] = useState<BrowseItem[]>([]); const [facets, setFacets] = useState<Facets | null>(null); const [hasMore, setHasMore] = useState(false); const [loading, setLoading] = useState(true); const [loadingMore, setLoadingMore] = useState(false);
  ...
  const fetchPage = useCallback(async (before: string | null, withFacets: boolean) => {
    ...
    return { ...result, items: groupBrowseItems(result.items ?? []) };
  }, [searchDebounced, sources, projectId, origin, category, type]);
  useEffect(() => { let cancelled = false; setLoading(true); setDetail(null); setSelectedIds(new Set()); setAnchorIdx(null); fetchPage(null, true).then(result => { if (!cancelled) { setItems(result.items); setHasMore(result.has_more); setFacets(result.facets); setLoading(false); } }); return () => { cancelled = true; }; }, [fetchPage]);
  const loadMore = async () => { if (loadingMore || !items.length) return; setLoadingMore(true); const result = await fetchPage(items[items.length - 1].created_at, false); setItems(previous => groupBrowseItems([...previous, ...result.items])); setHasMore(result.has_more); setLoadingMore(false); };
  ...
  const onItemPress = (item: BrowseItem) => { const index = items.findIndex(row => idOf(row) === idOf(item)); if (webModifierKeys.shift && anchorIdx !== null) { const [a, b] = [anchorIdx, index].sort((x, y) => x - y); setSelectedIds(previous => { const next = new Set(previous); for (let i = a; i <= b; i++) next.add(idOf(items[i])); return next; }); return; } if (isMultiSelectModifierActive()) { toggleOne(item, index); return; } setFastPreview(isDoubleTap(idOf(item))); setDetail(item); setAnchorIdx(index); };
  ...
  return <View className="flex-1"><View className="px-4 pt-4 flex-row items-center gap-2"><FilterChipGroup>{SOURCE_TABS.map((tab, index) => <TouchableOpacity key={tab.label} onPress={() => { setSourceTab(index); clearFilters(); }} className={`px-4 py-2 rounded-xl border ${sourceTab === index ? 'bg-brand-primary/10 border-brand-primary' : 'bg-surface-card border-surface-border'}`}><Text className={sourceTab === index ? 'text-brand-primary text-xs font-black' : 'text-typography-muted text-xs font-black'}>{tab.label}</Text></TouchableOpacity>)}</FilterChipGroup><View className="flex-1" /><FilterPanel isOpen={filtersOpen} onOpenChange={setFiltersOpen} activeCount={filterCount} trigger={({ toggle }) => <Tooltip label="Filters"><TouchableOpacity accessibilityLabel="Filters" onPress={toggle} className="h-10 w-10 items-center justify-center rounded-xl border border-surface-border bg-surface-card"><FontAwesome name="filter" size={13} color={colors.textMuted} /></TouchableOpacity></Tooltip>}>{filterBody}</FilterPanel></View><View className="flex-1 p-4">{collection}{hasMore && <View className="items-center pt-4"><TouchableOpacity onPress={loadMore} disabled={loadingMore} className="px-6 py-3 rounded-xl bg-surface-card border border-surface-border flex-row items-center gap-2">{loadingMore && <ActivityIndicator size="small" color={colors.primary} />}<Text className="text-typography-main font-black text-sm">Load more</Text></TouchableOpacity></View>}</View>{selectedItems.length > 0 && <View className="absolute bottom-4 left-4 right-4 rounded-2xl border border-brand-primary/30 bg-surface-card px-4 py-3 flex-row items-center gap-3"><Text className="text-brand-primary text-xs font-black">{selectedItems.length} selected</Text><View className="flex-1" /><TouchableOpacity disabled={zipping} onPress={downloadSelected} className="px-3 py-2 rounded-xl bg-brand-primary flex-row items-center gap-2">{zipping && <ActivityIndicator size="small" color="#fff" />}<Text className="text-white text-xs font-black">Download ZIP</Text></TouchableOpacity><TouchableOpacity onPress={clearSelection} className="p-2"><FontAwesome name="times" size={13} color={colors.textMuted} /></TouchableOpacity></View>}{detailFile && <View className={compact ? 'absolute inset-0 bg-surface-background p-4' : 'absolute inset-4 bg-surface-background border border-surface-border rounded-2xl p-4'}><FileHubDetailPane file={detailFile} onClose={() => setDetail(null)} compact={compact} autoPreview={fastPreview} /></View>}{shareSheet}</View>;
}

function BrowseRow({ item, colors, onDownload, onShare }: { item: BrowseItem; colors: { textMuted: string }; onDownload?: () => void; onShare?: () => void }) {
  return <View className="flex-row items-center gap-3 flex-1"><FontAwesome name={fileIcon(item.mime_type)} size={16} color={colors.textMuted} /><View className="flex-1 min-w-0"><Text numberOfLines={1} className="text-typography-main text-sm font-bold">{item.file_name}</Text><Text numberOfLines={1} className="text-typography-muted text-[11px]">{[item.project_name || item.task_title, getBrowseOriginLabel(item.origin), item.alias_count ? `${item.alias_count} alias${item.alias_count === 1 ? '' : 'es'}` : null, formatSize(item.size_bytes)].filter(Boolean).join(' · ')}</Text>{item.alias_context && <Text numberOfLines={1} className="text-typography-muted text-[10px]">Also in {item.alias_context}</Text>}</View>{onDownload && <Tooltip label="Download"><TouchableOpacity onPress={event => { event.stopPropagation(); onDownload(); }} className="w-8 h-8 rounded-lg items-center justify-center border border-surface-border"><FontAwesome name="download" size={11} color={colors.textMuted} /></TouchableOpacity></Tooltip>}{onShare && <Tooltip label="Share file"><TouchableOpacity onPress={event => { event.stopPropagation(); onShare(); }} className="w-8 h-8 rounded-lg items-center justify-center border border-surface-border"><FontAwesome name="share" size={11} color={colors.textMuted} /></TouchableOpacity></Tooltip>}</View>;
}
