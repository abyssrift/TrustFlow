...
export default function FileHubBrowse({ compact }: { compact?: boolean }) {
  ...
  const renderCard = (item: BrowseItem, _density: 'large' | 'medium') => <View pointerEvents="none"><FilePreviewCard fileName={item.file_name} mimeType={item.mime_type} subtitle={[item.project_name || item.task_title, getBrowseOriginLabel(item.origin), item.alias_count ? `${item.alias_count} alias${item.alias_count === 1 ? '' : 'es'}` : null, item.alias_context ? `Also in ${item.alias_context}` : null, formatSize(item.size_bytes)].filter(Boolean).join(' · ')} imageUri={isImage(item.mime_type) ? signedUrls[idOf(item)] : undefined} sizeBytes={item.size_bytes} onPress={() => onItemPress(item)} /></View>;
