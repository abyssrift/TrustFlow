...
assert.equal(getProjectWorkspaceLink({ origin: 'workspace', project_id: project, folder_id: folder, workspace_folder_id: '99999999-9999-4999-8999-999999999999', canonical_file_id: file }), `/projects/${project}?tab=files&folder=${folder}&file=${file}`);
assert.equal(getProjectWorkspaceLink({ origin: 'workspace', project_id: project, folder_id: '44444444-4444-4444-8444-444444444444', workspace_folder_id: folder, canonical_file_id: file }), `/projects/${project}?tab=files&folder=44444444-4444-4444-8444-444444444444&file=${file}`);
...
assert.equal(getProjectWorkspaceLink({ origin: 'workspace', project_id: project, folder_id: folder, canonical_file_id: `${file}%2Fother` }), null);
...
assert.match(source, /useMemo\([\s\S]*groupBrowseItems\(rawBrowseItems\)/);
assert.match(source, /const renderCard = \(\s*item: BrowseItem[\s\S]*pointerEvents="none"/);
