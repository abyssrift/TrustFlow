...
export type ApprovalQueueEntry = {
  ...
  /** Only present when the queue spans multiple tasks (e.g. a dashboard-wide review). */
  task?: { id: string; title: string } | null;
};

export function formatDeclaredMinutes(m: number) {
...
export default function ManualTimeApprovalsModal({ visible, onClose, entries, onReview, onNavigateToTask }: Props) {
  ...
  // Clamp position as entries are reviewed and removed from the queue.
  useEffect(() => {
    if (index > entries.length - 1) setIndex(Math.max(0, entries.length - 1));
  }, [entries.length, index]);

  // The queue emptying is the natural "done" signal — close instead of showing nothing.
  useEffect(() => {
      ...
      <View className="px-8 pb-10">
        ...
        <View className="flex-row items-center justify-between mb-6">
          <View className="flex-row items-center gap-3 flex-1">
            ...
            <View>
              <Text style={{ color: colors.textMain }} className="text-xl font-black tracking-tight">
                Time Declarations
              </Text>
              <Text style={{ color: colors.textMuted }} className="text-[10px] font-black uppercase tracking-widest mt-0.5">
        ...
        {/* Queue position dots — a real signal of where you are in review, not decoration */}
        {entries.length > 1 && (
        ...
        {/* Current entry */}
        <View style={{ backgroundColor: colors.background, borderColor: colors.border }} className="rounded-3xl border p-6 mb-6">
          <View className="flex-row items-center justify-between mb-4">
            <View className="flex-row items-center gap-2.5 flex-1 min-w-0">
              ...
              <Text style={{ color: colors.textMain }} className="font-black text-sm flex-1" numberOfLines={1}>
            ...
            {entry.task && (
              <TouchableOpacity
                ...
                className="bg-brand-primary/10 px-2.5 py-1 rounded-lg ml-2 flex-shrink-0"
              >
          ...
          <Text style={{ color: colors.textMain }} className="text-4xl font-black tracking-tighter mb-1">
            {formatDeclaredMinutes(entry.declared_minutes)}
          </Text>
          <Text style={{ color: colors.textMuted }} className="text-[10px] font-black uppercase tracking-widest mb-4">
            Declared
          </Text>

          {entry.reason && (
            <Text style={{ color: colors.textDim }} className="text-sm leading-5 italic mb-3">
              “{entry.reason}”
            </Text>
          )}
        ...
        {/* Manual paging — browse the queue without acting on it in strict order */}
        {entries.length > 1 && (
          <View className="flex-row items-center justify-between mb-6 px-2">
            ...
            >
              ...
              <Text style={{ color: colors.textMuted }} className="text-[10px] font-black uppercase tracking-widest">Prev</Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              className="flex-row items-center gap-2 py-2"
            >
              <Text style={{ color: colors.textMuted }} className="text-[10px] font-black uppercase tracking-widest">Next</Text>
        ...
        <View className="flex-row gap-4">
          <TouchableOpacity
            ...
            disabled={isLoading}
            className={`flex-1 py-4 rounded-2xl bg-state-danger/15 border border-state-danger/30 items-center justify-center active:opacity-75 ${loading === 'reject' ? 'opacity-60' : ''}`}
          >
            {loading === 'reject' ? (
          ...
          <TouchableOpacity
            ...
            disabled={isLoading}
            className={`flex-[2] py-4 rounded-2xl bg-state-success items-center justify-center shadow-lg ${loading === 'approve' ? 'opacity-60' : ''}`}
          >
            {loading === 'approve' ? (
