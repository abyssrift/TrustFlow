        ...
        {mobilePage === 'identity' ? (
          <>
            ...
            <View className="flex-row items-center justify-between px-5 pt-3 pb-5">
              ...
              <Tooltip label="Close">
                <TouchableOpacity onPress={requestClose} className="w-10 h-10 items-center justify-center rounded-full" style={{ backgroundColor: c.background, borderWidth: 1, borderColor: c.border }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="px-5" contentContainerStyle={{ paddingBottom: 24 }}>
                ...
                <>
                  ...
                  <View className="flex-row flex-wrap gap-2 mb-6">
                    {ROLE_TEMPLATES.map(tpl => (
                      <TouchableOpacity key={tpl.id} onPress={() => onApplyTemplate(tpl)} className="flex-row items-center px-3 py-2 rounded-xl" style={{ backgroundColor: c.background, borderWidth: 1, borderColor: name === tpl.name ? tpl.color : c.border }}>
              ...
              <TextInput value={name} onChangeText={onChangeName} editable={canEdit} placeholder="Role name" placeholderTextColor={c.textMuted} className="rounded-xl px-4 py-4 font-black text-sm mb-3" style={{ backgroundColor: c.background, borderWidth: 1, borderColor: c.border, color: c.textMain, opacity: !canEdit ? 0.5 : 1 }} />
              <TextInput value={description} onChangeText={onChangeDescription} editable={canEdit} placeholder="Description..." placeholderTextColor={c.textMuted} multiline numberOfLines={3} textAlignVertical="top" className="rounded-xl px-4 py-4 text-sm mb-5 h-24 leading-5" style={{ backgroundColor: c.background, borderWidth: 1, borderColor: c.border, color: c.textMain, opacity: !canEdit ? 0.5 : 1 }} />
              ...
              {/* Permissions nav row */}
              <TouchableOpacity
                onPress={() => setMobilePage('permissions')}
                className="flex-row items-center justify-between p-4 rounded-2xl"
                style={{ backgroundColor: c.card, borderWidth: 1, borderColor: c.border }}
              >
                <View className="flex-row items-center gap-3">
                  <View className="w-9 h-9 rounded-full items-center justify-center" style={{ backgroundColor: c.primary + '1A' }}>
                    <FontAwesome name="key" size={14} color={c.primary} />
                  </View>
                  <View>
                    <Text style={{ color: c.textMain }} className="font-black text-sm">Permissions</Text>
                    <Text style={{ color: c.textMuted }} className="text-[10px] font-bold mt-0.5">
                      {activeCount} selected{sensitiveActive > 0 ? ` · ${sensitiveActive} high-impact` : ''}
                    </Text>
                  </View>
                </View>
            ...
            <View className="flex-row gap-3 px-5 py-4" style={{ borderTopWidth: 1, borderTopColor: c.border }}>
              <TouchableOpacity onPress={requestClose} className="flex-1 py-4 rounded-xl items-center" style={{ backgroundColor: c.background, borderWidth: 1, borderColor: c.border }}>
                <Text style={{ color: c.textMuted }} className="font-black text-[11px] uppercase tracking-widest">Cancel</Text>
              </TouchableOpacity>
              {canEdit ? (
                <TouchableOpacity onPress={onSave} disabled={loading} className="flex-[2] py-4 rounded-xl items-center" style={{ backgroundColor: c.primary, opacity: loading ? 0.6 : 1 }}>
                  <Text className="text-white font-black text-[11px] uppercase tracking-widest">Save Role</Text>
                </TouchableOpacity>
              ) : editingRole && onClone && (
                <TouchableOpacity onPress={onClone} className="flex-[2] py-4 rounded-xl items-center" style={{ backgroundColor: c.primary }}>
          ...
          <>
            {/* Permissions header with back */}
            <View className="flex-row items-center px-3 pt-3 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: c.border }}>
              <TouchableOpacity onPress={() => setMobilePage('identity')} className="w-10 h-10 items-center justify-center rounded-full mr-2" style={{ backgroundColor: c.background }}>
                <FontAwesome name="chevron-left" size={14} color={c.textMain} />
              </TouchableOpacity>
              <View className="flex-1">
                <Text style={{ color: c.textMuted }} className="text-[9px] font-black uppercase tracking-[0.3em] mb-0.5">Role Editor</Text>
                <Text style={{ color: c.textMain }} className="font-black text-base tracking-tight">Permissions</Text>
              </View>
              <Text style={{ color: c.textMuted }} className="text-[10px] font-black mr-1">{activeCount} selected</Text>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="px-5 mt-3" contentContainerStyle={{ paddingBottom: 24, gap: 20 }}>
              {visibleCategories.length === 0 && (
                <Text style={{ color: c.textMuted }} className="text-xs font-bold text-center mt-6">No permissions match "{query}".</Text>
              )}
                  ...
                  <View key={cat}>
                    <View className="flex-row items-center mb-2.5">
                      ...
                      {canEdit && onBulkToggle && (
                        <TouchableOpacity onPress={() => onBulkToggle(catIds, !allSelected)} className="px-3 py-1.5 rounded-lg" style={{ backgroundColor: c.background, borderWidth: 1, borderColor: c.border }}>
                      ...
                      {catPerms.map(perm => {
                        ...
                        return (
                          <TouchableOpacity
                            ...
                            style={{ backgroundColor: isActive ? c.primary + '0D' : 'transparent', borderWidth: 1, borderColor: isActive ? c.primary + '66' : c.border, opacity: !canEdit ? 0.7 : 1 }}
                          >
