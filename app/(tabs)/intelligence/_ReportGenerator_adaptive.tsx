...
export default function ReportGenerator({
  ...
  const content = (
    <View className={isPage ? "flex-1 bg-surface-background" : "w-full max-w-2xl h-[90%] bg-surface-background rounded-[32px] border border-surface-border overflow-hidden premium-shadow glass-card"}>
      ...
      {/* Footer */}
      <View className={`px-6 py-6 border-t border-surface-border flex-row gap-4 ${isPage ? 'bg-surface-card' : 'bg-surface-card/50'} ${isPage ? 'pb-12' : ''}`}>
        {!isPage && (
          <Pressable
            onPress={onClose}
            disabled={loading}
        ...
        <Pressable
          ...
          className={`${isPage ? 'flex-1' : 'flex-[1.5]'} py-4 rounded-2xl transition-all items-center ${loading || pipelines.length === 0 ? 'bg-surface-border' : 'bg-brand-primary hover:bg-brand-primary-hover active:scale-95 shadow-lg shadow-brand-primary/20'}`}
        >
          {loading ? (
