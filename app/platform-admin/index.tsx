            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View className="flex-row gap-3 px-6 py-4">
                <View className="flex-1 bg-surface-card rounded-2xl p-3 border border-surface-border items-center">
                  <Text className="text-typography-main font-black text-lg">{fmtNumber(detail.stats.total_tasks)}</Text>
  ...
  return (
    ...
    >
      <View className="flex-row gap-3 px-4 pt-4 pb-3">
        <StatTile label="Tenants" value={fmtNumber(companies.length)} icon="building" sub="companies on platform" />
        <StatTile label="Users" value={fmtNumber(totalUsers)} icon="users" sub="across all workspaces" />
      </View>
      <View className="flex-row gap-3 px-4 pb-4">
        <StatTile label="Active Now" value={fmtNumber(liveCount)} icon="circle" accent={liveCount > 0} sub={liveCount > 0 ? 'sessions running' : 'no active sessions'} />
        <StatTile label="Usage / Week" value={fmtMins(totalMins)} icon="clock-o" sub={`${fmtNumber(totalTasks)} total tasks`} />
      </View>
      ...
      <View className="px-4 pt-4 pb-8">
        {timeline.length === 0 ? (
          ...
          timeline.slice().reverse().map(entry => {
            ...
            const displayVal = metric === 'sessions' ? fmtMins(val) : fmtNumber(val);
            return (
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      <View className="flex-row items-center justify-between px-4 pt-4 pb-3">
        <View className="flex-row items-center gap-2">
          <View className={`w-2 h-2 rounded-full ${sessions.length > 0 ? 'bg-state-success' : 'bg-surface-border'}`} />
