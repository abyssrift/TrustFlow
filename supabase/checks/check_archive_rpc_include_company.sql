-- Issue #431: regression coverage for the optional company-wide archive view.
-- Run after migrations with:
--   npx supabase db query --local -f supabase/checks/check_archive_rpc_include_company.sql
-- Fixture rows and role impersonation are transaction-scoped.
BEGIN;

CREATE TEMP TABLE archive_include_company_ctx (
  company_id uuid,
...
GRANT SELECT ON archive_include_company_ctx TO authenticated;
...
BEGIN
  ...
  VALUES ('#431 include-company check', 'archive-inc-' || replace(gen_random_uuid()::text, '-', ''))
  RETURNING id INTO c;
  INSERT INTO public.companies(name, slug)
  VALUES ('#431 foreign include-company check', 'archive-inc-' || replace(gen_random_uuid()::text, '-', ''))
  ...
  FOREACH u IN ARRAY ARRAY[caller, modifier_only, creator] LOOP
    INSERT INTO auth.users(
      ...
      'archive-inc-' || replace(u::text, '-', '') || '@example.invalid',
    ...
    VALUES (
      ...
      'archive-inc-' || replace(u::text, '-', '') || '@example.invalid',
      '#431 include-company check user', false
    );
  END LOOP;
  ...
  VALUES (c, '#431 include-company viewer') RETURNING id INTO caller_role;
  ...
  INSERT INTO public.roles(company_id, name)
  VALUES (c2, '#431 include-company modifier only') RETURNING id INTO modifier_role;
  INSERT INTO public.user_roles(user_id, role_id, company_id)
  ...
  INSERT INTO archive_include_company_ctx
...
DO $assertions$
DECLARE
  x archive_include_company_ctx%ROWTYPE;
  got_state text;
BEGIN
  SELECT * INTO x FROM archive_include_company_ctx;
