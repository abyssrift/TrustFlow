-- Fix SLA risk calculation: exclude sub-5-minute stage transitions from the
-- avg-dwell baseline. Seed/test data that moves tasks through stages in seconds
-- compresses avg_seconds to near-zero, causing every real task to exceed the
-- 1.5x threshold and show 99% risk. The 300-second floor removes that noise
-- while preserving legitimate historical dwell data.
...
BEGIN
  ...
  stage_dur_agg AS (
    ...
    WHERE ps.pipeline_id = COALESCE(p_pipeline_id, (
      SELECT pip.id FROM public.pipelines pip
      WHERE pip.company_id = v_company_id AND pip.is_default = TRUE LIMIT 1
    ))
  ...
  stage_avg_dwell AS (
    ...
    WHERE exited_at IS NOT NULL
      -- Exclude sub-5-minute transitions (seed/test noise) so they don't
      -- compress the baseline and flag every real task as at-risk.
