...
BEGIN
  ...
  IF COALESCE(v_requires_sub, FALSE) = TRUE OR COALESCE(v_requires_att, FALSE) = TRUE THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.users WHERE id = v_user_id AND is_owner = TRUE) THEN
