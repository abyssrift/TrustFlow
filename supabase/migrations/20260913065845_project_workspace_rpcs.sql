...
BEGIN
  ...
  IF v_project.workspace_folder_id IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM public.filehub_folders f WHERE f.id=v_project.workspace_folder_id AND f.deleted_at IS NULL) THEN
...
BEGIN
  IF NOT public.has_permission('project.view') THEN RAISE EXCEPTION 'Insufficient permissions to view projects.'; END IF;
  ...
  IF v_project.workspace_folder_id IS NOT NULL THEN
    WITH RECURSIVE workspace_tree AS (
      ...
      WHERE r.id=v_project.workspace_folder_id AND r.deleted_at IS NULL
      UNION ALL
...
DECLARE v_id UUID; v_parent public.filehub_folders%ROWTYPE; v_company UUID:=public.my_company_id(); v_user UUID:=auth.uid(); v_name TEXT:=trim(p_name);
BEGIN
  ...
  END IF;
  SELECT * INTO v_parent FROM public.filehub_folders
  WHERE id=p_parent_id AND company_id=v_company AND scope='project'
    AND project_id=p_project_id AND deleted_at IS NULL;
  IF NOT FOUND THEN RAISE EXCEPTION 'Workspace parent not found.'; END IF;
  SELECT id INTO v_id FROM public.filehub_folders
...
DECLARE a public.filehub_folders%ROWTYPE; b public.filehub_folders%ROWTYPE;
BEGIN
 SELECT * INTO a FROM public.filehub_folders WHERE id=p_id AND deleted_at IS NULL;
 ...
 IF a.scope='project' THEN
   ...
   SELECT * INTO b FROM public.filehub_folders WHERE id=p_new_parent_id AND deleted_at IS NULL;
   IF NOT FOUND OR NOT public.fn_project_workspace_descendant(p_new_parent_id,a.project_id) THEN RAISE EXCEPTION 'Workspace destination not found.'; END IF;
 ELSE
   IF NOT public.has_permission('filehub:view') THEN RAISE EXCEPTION 'Insufficient permissions.'; END IF;
 END IF;
 IF p_new_parent_id=p_id OR EXISTS(WITH RECURSIVE d AS (SELECT id FROM public.filehub_folders WHERE parent_id=p_id UNION ALL SELECT f.id FROM public.filehub_folders f JOIN d ON f.parent_id=d.id) SELECT 1 FROM d WHERE id=p_new_parent_id) THEN RAISE EXCEPTION 'Cannot move a folder into its own subfolder.'; END IF;
...
BEGIN
 SELECT * INTO f FROM public.filehub_folders WHERE id=p_id AND deleted_at IS NULL;
 IF NOT FOUND THEN RAISE EXCEPTION 'Folder not found.'; END IF;
 ...
 END IF;
 WITH RECURSIVE d AS (SELECT id FROM public.filehub_folders WHERE id=p_id UNION ALL SELECT x.id FROM public.filehub_folders x JOIN d ON x.parent_id=d.id)
 UPDATE public.filehub_folders SET deleted_at=now() WHERE id IN (SELECT id FROM d);
END; $$;
...
DECLARE f public.filehub_folders%ROWTYPE;
BEGIN
 SELECT * INTO f FROM public.filehub_folders WHERE id=p_id AND deleted_at IS NOT NULL AND deleted_at > now()-interval '15 days';
 ...
 END IF;
 WITH RECURSIVE d AS (SELECT id FROM public.filehub_folders WHERE id=p_id UNION ALL SELECT x.id FROM public.filehub_folders x JOIN d ON x.parent_id=d.id)
 UPDATE public.filehub_folders SET deleted_at=NULL WHERE id IN (SELECT id FROM d) AND deleted_at > now()-interval '15 days';
END; $$;

CREATE OR REPLACE FUNCTION public.rpc_filehub_file_move(p_file_id UUID,p_folder_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE f public.filehub_files%ROWTYPE; d public.filehub_folders%ROWTYPE;
BEGIN
 SELECT * INTO f FROM public.filehub_files WHERE id=p_file_id AND deleted_at IS NULL;
 ...
 IF f.visibility='project' THEN
   ...
   SELECT * INTO d FROM public.filehub_folders WHERE id=p_folder_id AND deleted_at IS NULL AND scope='project' AND project_id=f.project_id;
   IF NOT FOUND OR NOT public.fn_project_workspace_descendant(p_folder_id,f.project_id) THEN RAISE EXCEPTION 'Workspace destination not found.'; END IF;
 ELSE
   IF f.uploaded_by IS DISTINCT FROM auth.uid() THEN RAISE EXCEPTION 'File not found or you are not the uploader.'; END IF;
   IF p_folder_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM public.filehub_folders x WHERE x.id=p_folder_id AND x.company_id=f.company_id AND x.deleted_at IS NULL) THEN RAISE EXCEPTION 'Folder does not belong to this file''s context.'; END IF;
 END IF;
 UPDATE public.filehub_files SET folder_id=p_folder_id,updated_at=now(),updated_by=auth.uid() WHERE id=p_file_id;
END; $$;
...
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE f public.filehub_folders%ROWTYPE;
BEGIN SELECT * INTO f FROM public.filehub_folders WHERE id=p_id AND deleted_at IS NULL; IF NOT FOUND THEN RAISE EXCEPTION 'Folder not found.'; END IF; IF f.scope='project' THEN IF p_project_id IS DISTINCT FROM f.project_id OR NOT public.fn_project_mutation_accessible(f.project_id) OR f.project_root_kind IS NOT NULL OR NOT public.fn_project_workspace_descendant(f.id,f.project_id) THEN RAISE EXCEPTION 'Project folder cannot be renamed.'; END IF; ELSIF NOT public.has_permission('filehub:view') THEN RAISE EXCEPTION 'Insufficient permissions.'; END IF; UPDATE public.filehub_folders SET name=trim(p_name) WHERE id=p_id; END; $$;
...
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_id UUID; v_ver UUID; v_folder public.filehub_folders%ROWTYPE; v_company UUID:=public.my_company_id(); v_user UUID:=auth.uid(); v_name TEXT; v_size_limit BIGINT; v_storage_limit BIGINT; v_storage_used BIGINT; v_lock_key BIGINT;
BEGIN
 ...
 SELECT * INTO v_folder FROM public.filehub_folders WHERE id=p_folder_id AND company_id=v_company AND scope='project' AND project_id=p_project_id AND deleted_at IS NULL;
 IF NOT FOUND OR NOT public.fn_project_workspace_descendant(p_folder_id,p_project_id) THEN RAISE EXCEPTION 'Workspace destination not found.'; END IF;
...
BEGIN
 ...
 ELSE
   IF f.uploaded_by IS DISTINCT FROM auth.uid() THEN RAISE EXCEPTION 'File not found or you are not the uploader.'; END IF;
 END IF;
 UPDATE public.filehub_files SET deleted_at=now(),updated_at=now(),updated_by=auth.uid() WHERE id=p_file_id;
END; $$;
...
DECLARE f public.filehub_files%ROWTYPE;
BEGIN
 SELECT * INTO f FROM public.filehub_files WHERE id=p_file_id AND deleted_at IS NOT NULL AND deleted_at > now()-interval '15 days';
 ...
 END IF;
 UPDATE public.filehub_files SET deleted_at=NULL,updated_at=now(),updated_by=auth.uid() WHERE id=p_file_id;
END; $$;
...
BEGIN
 SELECT * INTO v FROM public.filehub_file_versions WHERE id=p_version_id;
 IF NOT FOUND THEN RAISE EXCEPTION 'Version not found.'; END IF;
 SELECT * INTO f FROM public.filehub_files WHERE id=v.file_id AND deleted_at IS NULL;
 IF NOT FOUND THEN RAISE EXCEPTION 'File not found.'; END IF;
 ...
 UPDATE public.filehub_file_versions SET superseded_at=now() WHERE file_id=f.id AND superseded_at IS NULL AND id<>v.id;
 UPDATE public.filehub_file_versions SET superseded_at=NULL WHERE id=v.id;
 UPDATE public.filehub_files SET current_version_id=v.id,storage_path=v.storage_path,original_name=v.original_name,size_bytes=v.size_bytes,mime_type=v.mime_type,content_hash=v.content_hash,updated_at=now(),updated_by=uid WHERE id=f.id;
END; $$;
...
BEGIN
 SELECT * INTO f FROM public.filehub_files WHERE id=p_target_id AND deleted_at IS NULL;
 IF NOT FOUND THEN RAISE EXCEPTION 'File not found.'; END IF;
 IF f.visibility<>'project' OR p_project_id IS DISTINCT FROM f.project_id OR NOT public.fn_project_mutation_accessible(f.project_id) OR f.folder_id IS NULL OR NOT EXISTS (SELECT 1 FROM public.filehub_folders x WHERE x.id=f.folder_id AND x.deleted_at IS NULL) OR NOT public.fn_project_workspace_descendant(f.folder_id,f.project_id) THEN RAISE EXCEPTION 'Project file is sealed or inaccessible.'; END IF;
 UPDATE public.filehub_file_versions SET superseded_at=now() WHERE file_id=f.id AND superseded_at IS NULL;
 ...
 UPDATE public.filehub_files SET current_version_id=vid,storage_path=p_storage_path,size_bytes=p_size_bytes,mime_type=p_mime_type,content_hash=p_content_hash,caption=COALESCE(NULLIF(trim(coalesce(p_caption,'')),''),caption),updated_at=now(),updated_by=auth.uid() WHERE id=f.id;
 RETURN vid;
END; $$;
