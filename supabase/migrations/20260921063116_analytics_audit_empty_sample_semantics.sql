-- Phase 2: preserve unavailable audit samples as JSON null.
-- Full replacement of the effective private implementation, rebased from
-- 20260816_fix_organizational_audit_throughput.sql. This replaces only the
-- function body under the existing private name and exact signature; its
-- SECURITY DEFINER mode, search_path, scope/filters, output shape, and the
-- public wrapper's authorization checks from
-- 20260915130000_reporting_rpc_authorization.sql remain unchanged.
--
-- Only the fallback-zero COALESCE is removed for current/comparison
-- success_rate, avg_lead_time_minutes, revision_rate, and
-- radar_advanced.flow_ratio / first_pass_yield. Their aggregate expressions and
-- denominators are unchanged, so empty samples become null while a measured
-- zero with an eligible denominator remains 0. Every other output stays as
-- defined by the rebased implementation.

CREATE OR REPLACE FUNCTION public._reporting_rpc_get_organizational_audit(
...
BEGIN
  ...
  cur_kpi AS (
    SELECT
      COUNT(CASE WHEN bt.terminal_type = 'success' THEN 1 END)                       AS throughput,
      COUNT(bt.id)                                                                  AS sample_size,
      ROUND(
        COUNT(CASE WHEN bt.terminal_type = 'success' THEN 1 END)::NUMERIC /
        NULLIF(COUNT(bt.id), 0) * 100, 2)                                        AS success_rate,
      ROUND(
        AVG(
      ...
      , 2)                                                                        AS avg_lead_time_minutes,
      ROUND(
        SUM(COALESCE(trf.had_revision, 0))::NUMERIC /
        NULLIF(COUNT(bt.id), 0) * 100, 2)                                        AS revision_rate
    FROM base_tasks bt
    LEFT JOIN task_rev_flag trf ON trf.task_id = bt.id
  ),
  prev_kpi AS (
    SELECT
      COUNT(DISTINCT CASE WHEN ps.terminal_type = 'success' THEN t.id END)           AS throughput,
      ROUND(
        COUNT(DISTINCT CASE WHEN ps.terminal_type = 'success' THEN t.id END)::NUMERIC /
        NULLIF(COUNT(DISTINCT t.id), 0) * 100, 2)                                AS success_rate,
      ROUND(
        AVG(
      ...
      ROUND(
        ...
        NULLIF(COUNT(DISTINCT t.id), 0) * 100, 2)                                AS revision_rate
    FROM public.tasks t
    JOIN public.pipeline_stages ps ON ps.id = t.current_stage_id
  ...
  adv_kpi AS (
    SELECT
      ROUND(
        COUNT(CASE WHEN bt.terminal_type = 'success' THEN 1 END)::NUMERIC /
        NULLIF(COUNT(bt.id), 0) * 100, 1)                                        AS flow_ratio,
      ROUND(
        COUNT(DISTINCT CASE WHEN COALESCE(trf.had_revision, 0) = 0 THEN bt.id END)::NUMERIC /
        NULLIF(COUNT(bt.id), 0) * 100, 1)                                        AS first_pass_yield
    FROM base_tasks bt
    LEFT JOIN task_rev_flag trf ON trf.task_id = bt.id
  ),
