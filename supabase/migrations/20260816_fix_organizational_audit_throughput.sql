-- Fix #265: rpc_get_organizational_audit's `throughput` counted every task
-- created in the window (an unfiltered COUNT), while its own sibling
-- `success_rate` (and the Throughput Trend chart's rpc_get_pipeline_
-- throughput_range) count only tasks that actually reached a
-- terminal_type = 'success' stage. The KPI tile and the chart below it
-- disagreed (116 vs 2) because they measured two different things under the
-- same name.
--
-- `throughput` now means "completed" in both cur_kpi and prev_kpi, so the
-- delta arrow (curThr - prevThr) stays a completions-vs-completions
-- comparison instead of comparing a total against... whatever prev_kpi was
-- counting (it had the same unfiltered bug).
--
-- Rebased on the full live definition from 20260718_sla_risk_multifactor.sql
-- (itself rebased on prod, not the intermediate 20260517 file) -- only the
-- two `throughput` lines in cur_kpi/prev_kpi changed, everything else is
