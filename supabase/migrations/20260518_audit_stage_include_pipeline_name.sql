-- Add pipeline_name to stage_duration_analysis and conversion_by_stage outputs
-- so the frontend can label which pipeline each stage belongs to when p_pipeline_id is NULL.
-- Also orders both arrays by (pipeline_name, position) for consistent grouping.
