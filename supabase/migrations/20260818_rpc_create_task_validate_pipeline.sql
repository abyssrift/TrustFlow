-- Fix #285: rpc_create_task trusted a client-supplied p_pipeline_id verbatim,
-- with no existence/company/deleted_at check. Cloning a task whose pipeline
-- had since been soft-deleted (CreateTaskModal.web.tsx handleCopyRecent copies
-- task.pipeline_id straight into the draft) silently created an orphaned task
-- pointing at a board no view can render. A dead/foreign id now falls through
-- the function's existing NULL-pipeline resolution (project's pipeline, then
-- company default) instead of being trusted directly.
