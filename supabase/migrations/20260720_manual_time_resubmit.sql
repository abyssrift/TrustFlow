-- ============================================================
-- Allow a worker to resubmit a manual time declaration even after
-- it was already approved/rejected (the "Declare Work Time" button
-- no longer hides once an entry exists — see TimerPanel.tsx).
--
-- Resubmitting must always go back through manager approval, so
-- rpc_log_manual_time now resets approval_status/rejection_reason/
-- approved_at/approved_by on conflict instead of leaving a stale
-- 'approved'/'rejected' status on the new numbers.
--
-- A resubmit-then-reapprove must not double-count: link the entry
-- to the work session it produced (session_id) so re-approval
-- updates that same session in place instead of inserting a
-- second one for the same declaration.
-- ============================================================

ALTER TABLE public.task_manual_time_entries
    ADD COLUMN IF NOT EXISTS session_id UUID REFERENCES public.task_work_sessions(id) ON DELETE SET NULL;
...
BEGIN
    ...
    ON CONFLICT (task_id, stage_id, user_id) DO UPDATE
        SET declared_minutes      = EXCLUDED.declared_minutes,
            ...
            logged_at             = now(),
            -- Resubmitting always needs a fresh manager decision, even if the
            -- previous declaration was already approved or rejected.
...
BEGIN
    ...
    IF p_approve THEN
        ...
        -- A resubmit of a previously-approved entry already has a session
        -- (session_id) — update it in place instead of inserting a second
        -- one, so re-approving doesn't double-count the declared time.
