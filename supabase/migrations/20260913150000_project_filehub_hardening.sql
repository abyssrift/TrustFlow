...
DECLARE v_project RECORD; v_workspace JSONB := jsonb_build_object('root',NULL,'folders','[]'::jsonb,'files','[]'::jsonb,'capabilities',jsonb_build_object('view',true,'upload',false,'create',false,'rename',false,'move',false,'delete',false,'restore',false)); v_root JSONB;
BEGIN
  ...
  IF v_project.workspace_folder_id IS NOT NULL THEN
    ...
    SELECT jsonb_build_object(
      ...
      'capabilities',jsonb_build_object('view',true,'upload',public.fn_project_mutation_accessible(p_project_id),'create',public.fn_project_mutation_accessible(p_project_id),'rename',public.fn_project_mutation_accessible(p_project_id),'move',public.fn_project_mutation_accessible(p_project_id),'delete',public.fn_project_mutation_accessible(p_project_id),'restore',public.fn_project_mutation_accessible(p_project_id))) INTO v_workspace;
  END IF;
