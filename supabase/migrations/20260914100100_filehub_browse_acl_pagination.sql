-- FileHub Browse pagination correction: filter by ACL before limiting.
-- Rerunnable replacement for the current cursor-aware Browse definition.
...
BEGIN
  ...
  WITH candidate AS (
    ...
    WHERE fi.company_id=v_company
      ...
      AND CASE WHEN fi.origin IN ('workspace','deliverable') THEN public.fn_project_accessible(fi.project_id)
        WHEN fi.source='filehub' THEN public.filehub_file_accessible(fi.file_id)
        ELSE public.fn_task_file_accessible(fi.task_id) END
    ORDER BY fi.created_at DESC, fi.file_id DESC
    LIMIT v_limit + 1
  ), acc AS (
  ...
  SELECT coalesce((SELECT jsonb_agg(to_jsonb(a) ORDER BY a.created_at DESC, a.file_id DESC) FROM acc a),'[]'::jsonb),
    (SELECT count(*) FROM candidate) INTO v_items,v_pool;
  ...
  RETURN jsonb_build_object('items',v_items,'has_more',v_pool > v_limit,'facets',v_facets);
END;
$$;
