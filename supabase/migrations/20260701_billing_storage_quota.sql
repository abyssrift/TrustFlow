-- Storage quota: track total bytes per company and enforce per-plan limits.

-- ─────────────────────────────────────────────────────────────
-- 1. Storage counter on company_billing
-- ─────────────────────────────────────────────────────────────
ALTER TABLE public.company_billing
  ADD COLUMN IF NOT EXISTS storage_used_bytes bigint NOT NULL DEFAULT 0;

-- Backfill from existing live files (deleted_at IS NULL).
UPDATE public.company_billing cb
SET storage_used_bytes = COALESCE((
  SELECT SUM(f.size_bytes)
  FROM public.filehub_files f
  WHERE f.company_id = cb.company_id AND f.deleted_at IS NULL
), 0);

-- ─────────────────────────────────────────────────────────────
-- 2. Add max_storage_bytes to billing_plans.limits
-- ─────────────────────────────────────────────────────────────
UPDATE public.billing_plans SET limits = limits || '{"max_storage_bytes": 524288000}'::jsonb   WHERE code = 'free';        -- 500 MB
UPDATE public.billing_plans SET limits = limits || '{"max_storage_bytes": 53687091200}'::jsonb  WHERE code = 'pro';         -- 50 GB
UPDATE public.billing_plans SET limits = limits || '{"max_storage_bytes": 536870912000}'::jsonb WHERE code = 'business';    -- 500 GB
UPDATE public.billing_plans SET limits = limits || '{"max_storage_bytes": null}'::jsonb         WHERE code = 'enterprise';  -- unlimited

-- ─────────────────────────────────────────────────────────────
-- 3. Helper: company storage limit in bytes (-1 = unlimited)
--    Falls back to 500 MB if no billing row.
-- ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public._company_storage_limit(p_company_id uuid)
RETURNS bigint LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT COALESCE(
    ...
    524288000  -- 500 MB default (free plan)
  );
$$;

-- ─────────────────────────────────────────────────────────────
-- 4. Trigger: keep storage_used_bytes in sync with filehub_files
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  ...
  ELSE
    IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
      -- soft-deleted: subtract
      v_delta := -COALESCE(NEW.size_bytes, 0);
    ELSIF OLD.deleted_at IS NOT NULL AND NEW.deleted_at IS NULL THEN
      -- restored: add back
      v_delta := COALESCE(NEW.size_bytes, 0);
    ELSE
      -- replace: net diff (may be 0)
...
CREATE TRIGGER filehub_storage_tracker
  AFTER INSERT OR UPDATE OF size_bytes, deleted_at ON public.filehub_files
  FOR EACH ROW EXECUTE FUNCTION public._filehub_storage_tracker();

-- ─────────────────────────────────────────────────────────────
-- 5. rpc_billing_overview: expose storage_used_bytes
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 6. rpc_filehub_upload_commit: add storage quota check
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 7. rpc_filehub_replace_file: add storage quota check (net delta)
-- ─────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- Total storage quota: only check if the new version is larger.
