-- Feature D: version control + soft delete for task brief attachments (Model B, D1-D2)
-- Plan: docs/SUBMISSION_VERSIONING_PLAN.md
-- A brief attachment is a standalone file -> exact FileHub Model B per file.

-- ── D1: task_attachment_versions (immutable file records) ───────────────────
...
-- UNIQUE(attachment_id, version_no) already indexes the hot lookup path.
CREATE INDEX IF NOT EXISTS idx_tav_superseded
  ON public.task_attachment_versions (superseded_at) WHERE superseded_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_tav_company
  ON public.task_attachment_versions (company_id);
...
-- Mirrors the parent's visibility gate (task_attachments_select = company only).
DROP POLICY IF EXISTS task_attachment_versions_select ON public.task_attachment_versions;
CREATE POLICY task_attachment_versions_select ON public.task_attachment_versions
  FOR SELECT USING (company_id = my_company_id());

-- ── D1: pointer + soft-delete + audit columns on task_attachments ───────────
ALTER TABLE public.task_attachments
  ADD COLUMN IF NOT EXISTS current_version_id UUID REFERENCES public.task_attachment_versions(id),
  ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS deleted_by UUID REFERENCES public.users(id),
  ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS updated_by UUID REFERENCES public.users(id);
-- file_name/file_url/file_size/mime_type/storage_path stay as denormalized
-- copies of the current version, so all current reads work unchanged.

CREATE INDEX IF NOT EXISTS idx_task_attachments_task_live
  ON public.task_attachments (task_id) WHERE deleted_at IS NULL;

-- ── D1: idempotent backfill (v1 per existing attachment) ────────────────────
WITH v1 AS (
  INSERT INTO public.task_attachment_versions
    (attachment_id, company_id, version_no, storage_path, file_name, file_size, mime_type, created_by, created_at, superseded_at)
  SELECT a.id, a.company_id, 1, a.storage_path, a.file_name, a.file_size, a.mime_type, a.uploaded_by, a.created_at, NULL
...
FROM   v1
WHERE  a.id = v1.attachment_id;

-- ── rpc_add_task_attachments: also create v1 + set pointer ──────────────────
-- Full re-apply of the live definition; only changes: v_version_id declared,
-- v1 version insert + current_version_id pointer after each attachment insert.
...
-- ── D2: rpc_replace_task_attachment — new version + supersede + pointer ─────
...
-- ── D2: rpc_task_attachment_versions — history, newest first ────────────────
-- Same per-version field names as rpc_submission_versions so shared UI renders both.
CREATE OR REPLACE FUNCTION public.rpc_task_attachment_versions(p_attachment_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ── D2: rpc_restore_task_attachment_version — pure pointer move ─────────────
CREATE OR REPLACE FUNCTION public.rpc_restore_task_attachment_version(p_version_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ── D2: rpc_delete_task_attachment — soft delete ─────────────────────────────
-- No delete path existed before (no RPC, no RLS delete policy, no client UI);
-- gate matches rpc_add_task_attachments (creator/manager/tasks.manage) + owner.
CREATE OR REPLACE FUNCTION public.rpc_delete_task_attachment(p_attachment_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ── D2: rpc_restore_task_attachment — management recovery ───────────────────
CREATE OR REPLACE FUNCTION public.rpc_restore_task_attachment(p_attachment_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ── D2: rpc_list_deleted_task_attachments — management only ─────────────────
-- Same JSONB shape as rpc_get_task_details v_task_attachments + deleted_at/deleted_by.
CREATE OR REPLACE FUNCTION public.rpc_list_deleted_task_attachments(p_task_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
GRANT EXECUTE ON FUNCTION public.rpc_task_attachment_versions(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_restore_task_attachment_version(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_delete_task_attachment(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_restore_task_attachment(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_list_deleted_task_attachments(uuid) TO authenticated;

-- ── D2: rpc_get_task_details — live brief attachments + version_count ───────
-- Full re-apply of the live definition (Feature B's deleted_at filters and
-- Feature A's version_count/current-version attachment filters preserved).
-- Only changes:
--   * has_attachment precondition EXISTS gains AND deleted_at IS NULL
--   * v_task_attachments filters a.deleted_at IS NULL and gains
