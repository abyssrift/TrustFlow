-- Feature A: version control + editing for task submissions (Model B, A1-A4)
-- Plan: docs/SUBMISSION_VERSIONING_PLAN.md
-- Decision #1: editing an approved/confirmed submission resets status to 'pending' (re-review).
-- Decision #2: edit perm = original submitted_by OR tasks.manage OR is_owner.

-- ── A1: task_submission_versions (immutable content records) ────────────────
...
-- UNIQUE(submission_id, version_no) already indexes the hot lookup path.
CREATE INDEX IF NOT EXISTS idx_tsv_superseded
  ON public.task_submission_versions (superseded_at) WHERE superseded_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_tsv_company
  ON public.task_submission_versions (company_id);
...
-- Mirrors submission_attachments_select (company + can view the parent submission).
DROP POLICY IF EXISTS task_submission_versions_select ON public.task_submission_versions;
CREATE POLICY task_submission_versions_select ON public.task_submission_versions
  FOR SELECT USING (
    company_id = my_company_id()
    AND EXISTS (
      SELECT 1 FROM public.task_submissions ts
      WHERE ts.id = task_submission_versions.submission_id
        AND (
          ts.submitted_by = auth.uid()
          OR has_permission('submission.review'::text)
          OR (SELECT users.is_owner FROM public.users WHERE users.id = auth.uid()) = true
        )
    )
  );

-- ── A1: pointer + audit columns on task_submissions ─────────────────────────
ALTER TABLE public.task_submissions
  ADD COLUMN IF NOT EXISTS current_version_id UUID REFERENCES public.task_submission_versions(id),
  ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS updated_by UUID REFERENCES public.users(id);
-- task_submissions.content stays as a denormalized copy of the current version's content.

-- ── A1: attachments belong to a version ──────────────────────────────────────
ALTER TABLE public.submission_attachments
  ADD COLUMN IF NOT EXISTS version_id UUID REFERENCES public.task_submission_versions(id);
CREATE INDEX IF NOT EXISTS idx_submission_attachments_version
  ON public.submission_attachments (version_id);

-- ── A1: idempotent backfill (v1 per existing submission) ────────────────────
WITH v1 AS (
  INSERT INTO public.task_submission_versions
    (submission_id, company_id, version_no, content, created_by, created_at, superseded_at)
  SELECT s.id, s.company_id, 1, s.content, s.submitted_by, s.submitted_at, NULL
...
FROM   v1
WHERE  s.id = v1.submission_id;

UPDATE public.submission_attachments a
SET    version_id = s.current_version_id
FROM   public.task_submissions s
WHERE  a.submission_id = s.id AND a.version_id IS NULL;

-- ── rpc_submit_work: also create v1 + set pointer + tag attachments ─────────
-- Full re-apply of the live definition; only changes: v_version_id declared,
-- v1 version insert + current_version_id pointer after the submission insert,
-- and version_id on the attachment inserts.
...
-- ── A2: rpc_edit_submission — new version on the same submission ────────────
...
-- ── A3: rpc_submission_versions — history, newest first ─────────────────────
CREATE OR REPLACE FUNCTION public.rpc_submission_versions(p_submission_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ── A4: rpc_restore_submission_version — pure pointer move ──────────────────
CREATE OR REPLACE FUNCTION public.rpc_restore_submission_version(p_version_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
GRANT EXECUTE ON FUNCTION public.rpc_submission_versions(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_restore_submission_version(uuid) TO authenticated;

-- ── A3: rpc_get_task_details — version_count + current-version attachments ──
-- Full re-apply of the live definition (Feature B's deleted_at filters preserved).
-- Only changes, both in v_submissions:
--   * 'version_count' + 'current_version_id' keys added
--   * attachments subquery filtered to a.version_id = s.current_version_id
--     (backfill above covers every legacy attachment row, so no NULL escape hatch)
...
BEGIN
  ...
  INTO v_task_attachments FROM public.task_attachments a WHERE a.task_id = p_task_id;
...
-- ── rpc_list_deleted_submissions — attachments filtered to current version ──
-- Full re-apply of the live (Feature B) definition; only change: attachments
-- subquery gains AND a.version_id = s.current_version_id.
CREATE OR REPLACE FUNCTION public.rpc_list_deleted_submissions(p_task_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
