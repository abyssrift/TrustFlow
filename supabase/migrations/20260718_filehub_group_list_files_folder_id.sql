-- 20260718_filehub_group_list_files_folder_id.sql
-- Bug: dragging a channel (group) file into a folder appeared to do nothing.
--
-- Channels list their files via rpc_filehub_group_list_files, and — unlike
-- inbox/sent/broadcast which filter by folder server-side (p_folder_id) — the
-- channel view filters CLIENT-side:
--     groupFiles.filter(f => (f.folder_id ?? null) === selectedFolderId)
-- But this RPC only ever returned a nested `folder` object ({id,name}), never
-- the flat `folder_id` the client reads. So f.folder_id was always undefined:
-- every channel file was pinned to the root, and a successful move (which does
-- update filehub_files.folder_id in the DB) re-fetched a payload that STILL
-- lacked folder_id, so the row never moved on screen.
--
-- Fix: expose f.folder_id as a top-level field, matching the FileHubFile type.
-- Only the returned JSON changes; no schema/permission changes.
