-- Submission gate modes: none | optional | required
--
-- Replaces the binary requires_submission gate with a 3-way mode, configurable
-- per stage from the pipeline builder.
--   required  -> advancement blocked until a submission exists (== old TRUE)
--   optional  -> submit form shown, advancement NOT blocked
--   none      -> no submit form at all, advancement NOT blocked
--
-- requires_submission stays as the server-side gate flag (rpc_advance_stage and
-- all read builders keep using it untouched). A trigger keeps the two consistent
-- across every write path (RPCs, rpc_create_pipeline presets, seeds), so
-- requires_submission == (submission_mode = 'required') is always true.

-- 1. Column + backfill (existing non-required stages -> 'none' per product call)
ALTER TABLE public.pipeline_stages
  ADD COLUMN IF NOT EXISTS submission_mode text
  CHECK (submission_mode IN ('none', 'optional', 'required'));

UPDATE public.pipeline_stages
SET submission_mode = CASE WHEN requires_submission THEN 'required' ELSE 'none' END
WHERE submission_mode IS NULL;

-- 2. Keep submission_mode <-> requires_submission consistent on every write.
CREATE OR REPLACE FUNCTION public.sync_stage_submission_mode()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_sync_stage_submission_mode ON public.pipeline_stages;
CREATE TRIGGER trg_sync_stage_submission_mode
  BEFORE INSERT OR UPDATE OF submission_mode, requires_submission
  ON public.pipeline_stages
  FOR EACH ROW EXECUTE FUNCTION public.sync_stage_submission_mode();

ALTER TABLE public.pipeline_stages ALTER COLUMN submission_mode SET NOT NULL;

-- 3. rpc_add_stage: accept p_submission_mode (trigger reconciles requires_submission).
DROP FUNCTION IF EXISTS public.rpc_add_stage(uuid, text, text, text, boolean, boolean, text, boolean, boolean, boolean, jsonb, integer);
...
-- 4. rpc_update_stage: accept p_submission_mode.
DROP FUNCTION IF EXISTS public.rpc_update_stage(uuid, text, text, text, boolean, boolean, text, boolean, boolean, boolean, uuid, jsonb, integer, boolean);
...
-- 5. rpc_get_task_details: expose submission_mode on current_stage + all_stages.
-- (Body copied from 20260712_submission_delete_notification.sql; only the two
--  stage jsonb_build_object calls gain 'submission_mode'.)
