-- Clamp task weight to 1..10 at the DB level (issue #136 follow-up).
-- Existing out-of-range rows (old default 0, and a few manual outliers) are
-- clamped in place; spawn_recursive_task copies parent weight so it needs no change.

UPDATE public.tasks SET weight = LEAST(10, GREATEST(1, weight)) WHERE weight < 1 OR weight > 10;

ALTER TABLE public.tasks ALTER COLUMN weight SET DEFAULT 1;
ALTER TABLE public.tasks ALTER COLUMN weight SET NOT NULL;
ALTER TABLE public.tasks ADD CONSTRAINT tasks_weight_1_10 CHECK (weight BETWEEN 1 AND 10);

-- rpc_create_task: default was 0 (would now violate the check); clamp input.
CREATE OR REPLACE FUNCTION public.rpc_create_task(p_title text, p_description text DEFAULT NULL::text, p_priority text DEFAULT 'medium'::text, p_due_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_pipeline_id uuid DEFAULT NULL::uuid, p_project_id uuid DEFAULT NULL::uuid, p_manager_id uuid DEFAULT NULL::uuid, p_category text DEFAULT NULL::text, p_weight bigint DEFAULT 1, p_visibility_permission text DEFAULT NULL::text, p_start_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_estimated_hours numeric DEFAULT NULL::numeric)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
