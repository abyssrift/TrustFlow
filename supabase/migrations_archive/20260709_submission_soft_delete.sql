-- Feature B: soft delete + management recovery for task submissions (B1-B4)
...
CREATE INDEX IF NOT EXISTS idx_task_submissions_task_live
  ON public.task_submissions (task_id) WHERE deleted_at IS NULL;

-- ── B2: rpc_delete_submission → soft delete (same permission check) ─────────
CREATE OR REPLACE FUNCTION public.rpc_delete_submission(p_submission_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- ── B4: restore (management only — NOT the submitter) ───────────────────────
CREATE OR REPLACE FUNCTION public.rpc_restore_submission(p_submission_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- ── B4: list soft-deleted submissions for a task (management only) ──────────
...
GRANT EXECUTE ON FUNCTION public.rpc_delete_submission(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_restore_submission(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_list_deleted_submissions(uuid) TO authenticated;

-- ── B3: filter soft-deleted rows out of rpc_get_task_details ────────────────
-- Full re-apply of the live definition; only changes:
--   * v_submissions WHERE gains AND s.deleted_at IS NULL
--   * has_pending_submission / no_pending_submission / has_approved_submission
--     precondition EXISTS checks gain AND deleted_at IS NULL
