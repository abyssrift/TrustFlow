-- Submission deletion → notify task manager + expose deleted count in task details stats.
-- 1. rpc_delete_submission emits task.submission_deleted notification event
-- 2. notification_rules row routes it to the task's manager (payload_user pattern)
-- 3. rpc_get_task_details stats gains deleted_submission_count for the UI badge

-- ── 1. rpc_delete_submission: emit notification event ─────────────────────────
...
-- ── 2. Notification rule: route to the task manager ───────────────────────────
INSERT INTO public.notification_rules (name, description, event_type, conditions, recipient_strategies, recipient_config, is_active, created_by)
SELECT 'Submission Deleted — Manager Alert',
       'Notifies the task manager when a submission is deleted so it can be reviewed/restored.',
       'task.submission_deleted', '{}'::jsonb, ARRAY['payload_user'], '{"payload_field": "manager_id"}'::jsonb, true,
       (SELECT created_by FROM public.notification_rules WHERE created_by IS NOT NULL LIMIT 1)
WHERE NOT EXISTS (
  SELECT 1 FROM public.notification_rules WHERE event_type = 'task.submission_deleted'
);

-- ── 3. rpc_get_task_details: add deleted_submission_count to stats ────────────
