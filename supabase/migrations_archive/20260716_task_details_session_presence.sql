-- Adds avatar_url + last_heartbeat_at to the work_sessions payload of
-- rpc_get_task_details, so the task-detail presence stack can show who is
-- working (avatar) and whether they are idle (stale heartbeat) without a
-- second client round-trip against task_work_sessions.
--
-- (Body copied from 20260713_stage_submission_mode.sql; only the work_sessions
--  jsonb_build_object gains 'last_heartbeat_at' + 'avatar_url'.)
